...
function EditorExtension({ editor }) {
  ...
  const onAiClick = async () => {
    ...
    try {
      ...
      const PROMPT = `For question: "${selectedText}" and with the given content as an answer, please provide an appropriate response in HTML format. Answer content: ${AllUnformattedAns}`;

      const AiModelResult = await chatSession.sendMessage(PROMPT);
      const responseText = await AiModelResult.response.text(); // Corrected
  ...
  return (
    <div className="button-group">
      <div className="control-group flex gap-20">
        {editor && ( // Ensure editor is available before rendering buttons
          <div>
            <button
              onClick={() => editor.chain().focus().toggleBold().run()}
              className={editor.isActive("bold") ? "text-red-600" : ""}
